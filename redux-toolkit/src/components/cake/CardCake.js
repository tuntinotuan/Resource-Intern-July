...
const CardCake = ({ id, urlImg, title, isFavorites, props }) => {
  ...
  return (
    ...
    >
      <IconHeart
        className={`absolute top-4 right-4 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-700 z-50 ${
      ...
      <div className="relative">
        ...
        <button
          ...
          className="absolute bottom-2 left-1/2 -translate-x-1/2 text-darkMain bg-white shadow-md rounded-md p-3 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-700"
        >
          Add to card
        </button>
      </div>
